...
import (
	...
	transformer_utils "github.com/Jeppzify/neosync/worker/pkg/benthos/transformers/utils"
...
func Test_TransformEmailTransformer_InvalidEmailArg(t *testing.T) {
	mapping := fmt.Sprintf(`root = transform_email(value:%q,invalid_email_action:"passthrough")`, "nick@example.com")
	ex, err := bloblang.Parse(mapping)
	require.NoError(t, err, "failed to parse the email transformer")
...
func Test_TransformEmailTransformer_NoOptions(t *testing.T) {
	mapping := fmt.Sprintf(`root = transform_email(value:%q)`, "nick@example.com")
	ex, err := bloblang.Parse(mapping)
	require.NoError(t, err, "failed to parse the email transformer")
